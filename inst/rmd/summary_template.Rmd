...
_lofreq_top_outbreak_results.Rds | R data | The Outbreak API results from the top lofreq iSNVs (tibble)
_ivar_top_outbreak_results.Rds | R data | The Outbreak API results from the top ivar iSNVs (tibble)
...
```
# LEGACY CODE, no longer implemented
Renaming snpSIFT columns to match iVar output.. 
...
```{r, eval = F}
# LEGACY CODE FOR WHEN USING THE TSVs... no longer used

# files = list.files(path = summary_output_file_path, pattern = "*.ivar.final.masked.variants.tsv$", recursive = TRUE, full.names = T)
# 
# df = sapply(files, read_delim, delim = "\t", comment = "##", simplify = FALSE) %>% 
#   bind_rows(.id = "id") %>%
#   mutate(FILE = basename(id)) %>% 
#   mutate(SAMPLE = str_replace(FILE, ".ivar.final.masked.variants.tsv", "")) %>%
#   mutate(SAMPLE = str_replace(SAMPLE, "LLNL_", ""))
# 
# df = df %>%
#   select(-id, -REGION, -FILE) %>%
#   rename("GENE" = "GFF_FEATURE") %>%
#   unique()%>%
#   mutate(PIPELINE = "ivar")
# 
...
#   filter(PASS == TRUE) %>%
#   filter(ALT_FREQ >= 0.01) %>%
#   filter(ALT_QUAL > 20) %>%
#   filter(REF_AA != ALT_AA) %>%
#   select(-PASS, -REF_QUAL, -ALT_QUAL, -REF_RV, -ALT_RV)
# 
# ivar_genes = c("cds-YP_009724389.1",
#          "cds-YP_009725295.1",
#          "cds-YP_009724390.1",
#          "cds-YP_009724391.1",
#          "cds-YP_009724392.1",
...
#            )
# 
# replacements = data.frame("ivar" = ivar_genes, "lofreq" = lofreq_genes)
# 
# replacements %>% format_dt()
# 
# df_ivar = df_ivar %>%
#   mutate(GENE = str_replace_all(GENE, setNames(lofreq_genes, ivar_genes)))
```
...
get_SNV = function(SNV) {
  ...
  CA = tryCatch(data.frame(fromJSON(paste0('https://api.outbreak.info/genomics/prevalence-by-location?location_id=USA_US-CA&mutations=', SNV))),
          error = function(e)
          data.frame("success" = TRUE))
  
  USA = tryCatch(data.frame(fromJSON(paste0('https://api.outbreak.info/genomics/prevalence-by-location?location_id=USA&mutations=', SNV))),
          error = function(e)
          data.frame("success" = TRUE))
  
  GLOBAL = tryCatch(data.frame(fromJSON(paste0('https://api.outbreak.info/genomics/prevalence-by-location?mutations=', SNV))),
          error = function(e)
          data.frame("success" = TRUE))
  
  l = list("CA" = data.frame(CA), 
            "USA" = data.frame(USA), 
            "GLOBAL" = data.frame(GLOBAL))
  
  df = bind_rows(l, .id = "LOCATION")
  names(df)
  SNV2 = gsub(x = SNV, pattern = ":", replacement = ".") 
  names(df) = gsub(x = names(df), pattern = paste0("results.", SNV2, "."), replacement = "") 
...
results = lapply(top_lofreq, get_SNV)
saveRDS(results, file = file.path(summary_output_file_path, "_lofreq_top_outbreak_results.Rds"))
...
results = lapply(top_ivar, get_SNV)
saveRDS(results, file = file.path(summary_output_file_path, "_ivar_top_outbreak_results.Rds"))
```

```{r, fig.width=12, fig.height=12}
lofreq_results = readRDS(file = file.path(summary_output_file_path, "_lofreq_top_outbreak_results.Rds"))

a = bind_rows(lofreq_results) %>% 
  filter(total_count > 0) %>%
  group_by(SNV) %>%
  mutate(adj_proportion = proportion / max(proportion)) %>%
  ggplot(aes(x = date, y = SNV, fill = LOCATION, color = LOCATION, height = adj_proportion)) +
    jak_theme() +
    ggridges::geom_ridgeline(alpha = 0.2, size = 0.5) +
    scale_fill_manual(values = location_palette) +
    scale_color_manual(values = location_palette) +
    labs(title = "Lofreq Proportions (free y-axis scale)") + 
    scale_x_date(date_breaks = "months" , date_labels = "%b-%y")

ivar_results = readRDS(file = file.path(summary_output_file_path, "_ivar_top_outbreak_results.Rds"))

b = bind_rows(ivar_results) %>% 
  filter(total_count > 0) %>%
  group_by(SNV) %>%
  mutate(adj_proportion = proportion / max(proportion)) %>%
  ggplot(aes(x = date, y = SNV, fill = LOCATION, color = LOCATION, height = adj_proportion)) +
    ...
    scale_fill_manual(values = location_palette) +
    scale_color_manual(values = location_palette) +
    labs(title = "ivar Proportions (free y-axis scale)") + 
    scale_x_date(date_breaks = "months" , date_labels = "%b-%y")
...
c = bind_rows(lofreq_results) %>% 
  ...
  ggplot(aes(x = date, y = 100 * proportion, fill = LOCATION, color = LOCATION)) +
    ...
    scale_fill_manual(values = location_palette) +
    scale_color_manual(values = location_palette) +
    facet_wrap(~SNV, ncol = 2, scales = "free_y") + 
    labs(title = "Lofreq Proportions (free y-axis scale)", y = "Proportion (%)", x = "Month-Year") + 
    scale_x_date(date_breaks = "months" , date_labels = "%b-%y")

ivar_results = readRDS(file = file.path(summary_output_file_path, "_ivar_top_outbreak_results.Rds"))

d = bind_rows(ivar_results) %>% 
  filter(total_count > 0) %>%
  group_by(SNV) %>%
  ggplot(aes(x = date, y = 100 * proportion, fill = LOCATION, color = LOCATION)) +
    jak_theme() +
    ...
    scale_color_manual(values = location_palette) +
    facet_wrap(~SNV, ncol = 2, scales = "free_y") + 
    labs(title = "ivar Proportions (free y-axis scale)", y = "Proportion (%)", x = "Month-Year") + 
    scale_x_date(date_breaks = "months" , date_labels = "%b-%y")
...
c = bind_rows(lofreq_results) %>% 
  ...
  group_by(SNV) %>%
  mutate(adj_proportion = proportion / max(proportion)) %>%
  ggplot(aes(x = date, y = SNV, fill = LOCATION, color = LOCATION, height = proportion)) +
    jak_theme() +
    ggridges::geom_ridgeline(alpha = 0.2, size = 0.5, scale = 1) +
    scale_fill_manual(values = location_palette) +
    scale_color_manual(values = location_palette) +
    labs(title = "Lofreq Proportions (fixed y-axis scale)") + 
    scale_x_date(date_breaks = "months" , date_labels = "%b-%y")
...
d = bind_rows(ivar_results) %>% 
  ...
  ggplot(aes(x = date, y = SNV, fill = LOCATION, color = LOCATION, height = proportion)) +
    ...
    scale_fill_manual(values = location_palette) +
    scale_color_manual(values = location_palette) +
    labs(title = "ivar Proportions (fixed y-axis scale)") + 
    scale_x_date(date_breaks = "months" , date_labels = "%b-%y")
...
```{r, fig.width=12, fig.height=10}
m = bind_rows(lofreq_results) %>%
      rbind(bind_rows(ivar_results)) %>%
