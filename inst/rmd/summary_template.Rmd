...
COI_COV = readxl::read_excel(system.file("extdata", "muatation list for VOC and VOI 11.16.21.xlsx", package = "MappgeneSummary"), col_names = FALSE)
colnames(COI_COV) = c("GENE", "COI_COV", "AA_RANGE")

COI_COV = COI_COV %>%
  unique() %>%
  select("GENE", "COI_COV", "AA_RANGE") %>%
  separate(AA_RANGE, sep = "/", into = c("A", "B"), remove = F) %>%
  mutate(B = ifelse(is.na(B), A, B)) %>%
  transmute(GENE, COI_COV, AA_RANGE, AA_POS = map2(A, B, seq, by = 1)) %>%
  unnest(cols = AA_POS) %>%
  mutate(CODE = paste(GENE, AA_POS, sep = "_")) %>%
  select(-GENE, -AA_POS)

df_COI_COV = readRDS(file = file.path(summary_output_file_path, "_summary_DF.Rds")) %>%
