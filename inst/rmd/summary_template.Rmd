...
# Use mappygene to populate an NSP column
df_lofreq <- mappgene_summary_populate_df_col(df_lofreq, nsp_only=TRUE, out_type='str')
```
...
# Use mappygene to populate an NSP column
df_ivar <- mappgene_summary_populate_df_col(df_ivar, nsp_only=TRUE, out_type='str')
```
...
as = df %>%
  ...
  filter(EFFECT %in% keeper_effects) %>%
  select(SAMPLE, PIPELINE, POS, SAMPLE, REF, ALT, AF, GENE, NSP, REF_AA, ALT_AA, AA_POS, SNV) %>%
  group_by(PIPELINE, POS, REF, ALT, GENE, NSP, REF_AA, ALT_AA, AA_POS, SNV) %>%
...
ar = df %>%
  filter(EFFECT %in% keeper_effects) %>%
  select(POS, SAMPLE, PIPELINE, REF, ALT, DP, AF, GENE, NSP, REF_AA, ALT_AA, AA_POS, SNV)
...
# move this back to the text when the API is working again
...
df %>%
  ...
  arrange(desc(AF)) %>%
  select(SAMPLE, EFFECT, API_CODE, SNV, PIPELINE, GENE, NSP, POS, REF, ALT, AF, REF_AA, ALT_AA) %>%
  format_dt()
```
...
VOI_VOC_OTHER = readxl::read_excel(system.file("extdata", "mutation list for VOC and VOI 12.02.21v2.xlsx", package = "MappgeneSummary"), sheet = "Sheet1", col_names = FALSE)
colnames(VOI_VOC_OTHER) = c("GENE", "VOI_VOC", "AA_RANGE")
VOI_VOC_OTHER = VOI_VOC_OTHER %>%
  unique() %>%
  select("GENE", "VOI_VOC", "AA_RANGE") %>%
  mutate(TYPE = "Other")

VOI_VOC_OMICRON = readxl::read_excel(system.file("extdata", "mutation list for VOC and VOI 12.02.21v2.xlsx", package = "MappgeneSummary"), sheet = "omicron", col_names = FALSE)
colnames(VOI_VOC_OMICRON) = c("GENE", "VOI_VOC", "AA_RANGE")
VOI_VOC_OMICRON = VOI_VOC_OMICRON %>%
  unique() %>%
  #select("GENE", "VOI_VOC", "AA_RANGE") %>%
  mutate(TYPE = "OMICRON")

VOI_VOC = rbind(VOI_VOC_OTHER, VOI_VOC_OMICRON)
...
VOI_VOC = VOI_VOC %>%
  ...
  transmute(GENE, VOI_VOC, AA_RANGE, TYPE, AA_POS = map2(A, B, seq, by = 1)) %>%
  unnest(cols = AA_POS) %>%
...
df_VOI_VOC %>%
  ...
  group_by(GENE, NSP, VOI_VOC, TYPE, AA_RANGE, AA_POS, REF_AA,  ALT_AA, HGVS_C, HGVS_P, PIPELINE) %>%
